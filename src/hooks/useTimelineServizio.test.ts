...
/** I passi veri di LEED BD+C, nell'ordine in cui stanno a sistema. */
const LEED_BDC = [
...
describe("proponiPasso", () => {
  ...
  it("la firma non è un passo: si aggancia al primo della timeline", () => {
    expect(proponiPasso(LEED_BDC, "firma")).toBe(1);
    expect(proponiPasso(ENERGY, "firma")).toBe(1);
  });

  it("lascia vuoto invece di indovinare quando il momento non esiste", () => {
    // La timeline Energy non ha una fine costruzione né una sottomissione:
    // proporre un passo a caso qui vorrebbe dire agganciare una fattura a
    // un'attività sbagliata, che è peggio di non agganciarla.
    expect(proponiPasso(ENERGY, "costruzione")).toBeNull();
    expect(proponiPasso(ENERGY, "sottomissione")).toBeNull();
    expect(proponiPasso(ENERGY, "design")).toBeNull();
  });
