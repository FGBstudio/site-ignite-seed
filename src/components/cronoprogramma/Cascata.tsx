...
import { FONTI_SUGGERITE } from "@/types/cronoprogramma";
import {
...
/**
 * Il GC comunica nuove date.
 *
 * L'anteprima viene prima di qualunque scrittura: il PM vede cosa si muove,
 * cosa resta fermo, il verdetto sul contratto e chi verra' avvisato, e solo
 * dopo decide. E' la funzione che rende il meccanismo utile invece che un
 * adempimento — senza, spostare una data e' un salto nel buio.
 */
export function AggiornaHandover({
  eventoId,
  dataAttuale,
  certIdCorrente,
  certIdProprie,
  ...
  const { data: anteprima } = useCascataAnteprima(certIdCorrente, chiesta);
  const applica = useApplicaSpostamento();

  const delta =
    chiesta && dataAttuale
      ? Math.round((parseISO(chiesta).getTime() - parseISO(dataAttuale).getTime()) / 86400000)
      : 0;
    ...
    <Card className="p-5">
      <div className="mb-4">
        <div className="flex items-center gap-2.5">
          ...
          <h2 className="text-sm font-medium">Il GC comunica nuove date</h2>
        </div>
        <p className="mt-1.5 max-w-[70ch] text-xs text-muted-foreground">
          Sposti l'handover sul cronoprogramma: vedi l'anteprima di cosa si muove, poi confermi.
          Cambiano solo le attivita' ancorate all'handover.
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-end gap-2">
        ...
        <div className="flex-1 min-w-[200px]">
          ...
          <Input
            ...
            placeholder="es. gantt rev. 8 del GC, 12 marzo"
            className="h-9 text-xs"
          />
          <datalist id="fonti-cascata">
      ...
      {chiesta && anteprima && (
        <div className="mt-4 rounded-xl border border-primary/30 bg-primary/5 p-4">
          <p className="mb-3 text-xs font-medium text-primary">
            Handover {delta > 0 ? "+" : ""}
            {delta} giorni · {d(dataAttuale)} <ArrowRight className="inline h-3 w-3" /> {d(chiesta)}
          </p>
          ...
          {nomiAltri.length > 0 && (
            <p className="mt-2 flex items-start gap-1.5 text-[11px] text-muted-foreground">
              ...
              {nomiAltri.join(", ")} {nomiAltri.length === 1 ? "ricevera'" : "riceveranno"} la
              proposta per le proprie date: {nomiAltri.length === 1 ? "la conferma" : "le confermano"} chi
              di competenza, non tu.
            </p>
          )}
          ...
          <div className="mt-3 flex gap-2">
            <Button
              size="sm"
              disabled={!fonte.trim() || applica.isPending}
              onClick={async () => {
                try {
                  ...
                  toast({
                    ...
                    description: `${n} timeline aggiornate. Le altre restano in attesa del loro PM.`,
                  });
                } catch (e: any) {
            ...
            <Button size="sm" variant="ghost" onClick={() => setChiesta(null)}>
              Annulla
            </Button>
          </div>
          {!fonte.trim() && (
            <p className="mt-1.5 text-[11px] text-amber-700">
              Serve la fonte: e' cio' che permette a chi legge di sapere se ha informazioni piu'
              fresche o piu' vecchie.
            </p>
          )}
        </div>
      )}
  ...
  return (
    <Card className="border-amber-300 bg-amber-50/50 p-5 dark:border-amber-800 dark:bg-amber-950/20">
      ...
      <p className="mb-4 text-xs text-muted-foreground">
        Qualcuno ha spostato una data di cantiere. Le tue milestone non si sono mosse: decidi tu.
      </p>
          ...
          >
            <div className="min-w-0">
              ...
              <p className="text-[11px] text-muted-foreground">
                {c.sito} · {d(c.data_precedente)} <ArrowRight className="inline h-3 w-3" />{" "}
                {d(c.data_nuova)} · fonte: {c.fonte} · {c.milestone_da_spostare} milestone
              </p>
            </div>
            <Button
              size="sm"
 ...
 * Per questo ogni voce esce gia' coi numeri dentro — giorni, fine stimata,
 * scadenza, report — invece di essere una cronologia da cui ricostruirli.
 */
export function Registro({ cronoId }: { cronoId: string | undefined }) {
    ...
    <Card className="p-5">
      <div className="mb-4">
        <div className="flex items-center gap-2.5">
          ...
          <h2 className="text-sm font-medium">Registro del cronoprogramma</h2>
        </div>
        <p className="mt-1.5 text-xs text-muted-foreground">
          Ogni modifica: chi, quando, da dove, e cosa comporta.
        </p>
      </div>
        ...
        {voci.map((v) => {
          ...
          return (
            <div key={v.id} className="border-l-2 border-border pl-3">
              <p className="text-sm">
                {v.note && <b>{v.note}</b>} — handover dal {d(v.data_precedente)} al{" "}
                {d(v.data_nuova)}
              </p>
              <p className="mt-0.5 text-[11px] text-muted-foreground">
                {format(parseISO(v.quando), "d LLL yy, HH:mm", { locale: it })} · {v.fonte}
                {v.scostamento_giorni !== null && ` · ${v.scostamento_giorni > 0 ? "+" : ""}${v.scostamento_giorni} gg`}
