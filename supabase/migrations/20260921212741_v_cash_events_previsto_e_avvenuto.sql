-- La vista porta ora anche l'anello dell'evento, e la certezza usa il
-- vocabolario nuovo: e' quella che decide pieno o tratteggiato.
...
select
  ...
  -u.importo_eur,
  case
    when u.data_effettiva is not null and u.stato = 'pagata' then 'reale'
...
comment on view public.v_cash_events is
  'Ogni movimento di cassa, entrate e uscite, nella stessa forma. Porta tre date: l evento che lo fa maturare, il documento, la cassa. La certezza dice quale dei tre anelli e avvenuto davvero.';
